...
const Alert = ({ type, children, onClose }) => {
  return (
    <div
      className={`flex items-center p-4 mb-4 border-t-4 fixed top-0 right-1/2 translate-x-1/2 z-30 ${
        type === "success" 
    ...
    >
      {type === "success" && <MdDone className="w-5 h-5 shrink-0 text-green-800" />}
      {type === "error" && <MdOutlineErrorOutline className="w-5 h-5 shrink-0 text-red-800" />}
      {!type && <IoIosInformationCircleOutline className="w-5 h-5 shrink-0 text-blue-800" />}
